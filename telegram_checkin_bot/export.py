...
def export_messages(start_date, end_date):
    ...
    start_time = pd.to_datetime(start_date + " 00:00:00")
    end_time = pd.to_datetime(end_date + " 23:59:59")
    ...
    for _, row in photo_df.iterrows():
        ...
        if os.path.exists(src):
            ...
            filename = f"{row['timestamp'].strftime('%Y-%m-%d')}_{safe_username}_{row['keyword']}.jpg"
