...
def export_images(start_datetime: datetime, end_datetime: datetime):
    ...
    for date_str, group in photo_df.groupby(photo_df["timestamp"].dt.strftime("%Y-%m-%d")):
        current_zip_idx = 1
        current_zip_size = 0
        current_zip_path = os.path.join(export_dir, f"{date_str}_包{current_zip_idx}.zip")
        current_zip = zipfile.ZipFile(current_zip_path, "w", zipfile.ZIP_DEFLATED)

        for _, row in group.iterrows():
            pid = row["public_id"]
            ts_local = row["timestamp"].astimezone(BEIJING_TZ)
            keyword = row.get("keyword", "无关键词") or "无关键词"
            name = row.get("name", "未知") or "未知"

            filename = safe_filename(f"{ts_local.strftime('%H%M%S')}_{keyword}_{name}.jpg")
