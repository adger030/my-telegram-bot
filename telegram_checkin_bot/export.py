...
def _mark_late_early(excel_path: str):
    ...
    for sheet in wb.worksheets:
        for row in sheet.iter_rows(min_row=2):  # 跳过表头
            ...
            shift_name = re.split(r'[（(]', shift_text)[0]  # 提取班次名称（去掉标记）

            # 补卡标记（黄色）
            if "补卡" in shift_text:
                time_cell.fill = fill_yellow
                shift_cell.fill = fill_yellow
                if not shift_text.endswith("（补卡）"):
                    shift_cell.value = f"{shift_name}（补卡）"
                continue
            ...
            if shift_name in SHIFT_TIMES:
                ...
                if keyword_cell.value == "#上班打卡":
                    ...
                    if shift_name == "I班":
                        if dt.hour == 0:
                            continue  # 00:xx 正常下班
                        elif dt.time() < end_time:  # 23:xx 早退
                            time_cell.fill = fill_red
                            shift_cell.fill = fill_red
                    else:
                        if dt.time() < end_time:  # 其他班次早退
                            time_cell.fill = fill_red
                            shift_cell.fill = fill_red
