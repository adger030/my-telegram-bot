    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            if "user_id" not in columns:
                ...
                print("✅ 已为 messages 表添加 user_id 字段")
            ...
            # 创建 users 表（user_id 主键，username 唯一）
            cur.execute("""
...
def has_user_checked_keyword_today(user_id=None, username=None, keyword=None, day_offset=0):
    """检查用户当天(或指定偏移日)是否打过指定关键词"""
...
def get_user_logs(user_id=None, username=None, start=None, end=None):
    with get_conn() as conn:
        with conn.cursor() as cur:
            if user_id:
                cur.execute("""
...
def get_user_month_logs(user_id=None, username=None):
    now = datetime.now(BEIJING_TZ)
    start = now.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
    end = (start.replace(day=28) + timedelta(days=4)).replace(day=1)
    return get_user_logs(user_id=user_id, username=username, start=start, end=end)
...
def save_shift(user_id=None, username=None, shift=None):
    with get_conn() as conn:
        with conn.cursor() as cur:
            if user_id:
                cur.execute("""
...
def get_today_shift(user_id=None, username=None):
...
def get_user_name(user_id=None, username=None):
    with get_conn() as conn:
        with conn.cursor() as cur:
            if user_id:
